...
selectCollection1.onChange(function(selectedCollection) {
  ...
  updateNitrogenLegendVisibility();
});
...
// Show/hide the nitrogen legend and update its min/max labels
function updateNitrogenLegendVisibility(vp1, vp2) {
  var isCNC = function(col) {
    return col && String(col).indexOf('DP3.30018.002') !== -1;
  };
  var isCNC1 = isCNC(selectCollection1.getValue());
  var isCNC2 = isCNC(selectCollection2.getValue());
  var show = isCNC1 || isCNC2;
  percentNLegendPanel.style().set('shown', show);
  if (show && vp1 && isCNC1) {
    percentNMinLabel.setValue(vp1.min.toFixed(2) + '%');
    percentNMaxLabel.setValue(vp1.max.toFixed(2) + '%');
  } else if (show && vp2 && isCNC2) {
    percentNMinLabel.setValue(vp2.min.toFixed(2) + '%');
    percentNMaxLabel.setValue(vp2.max.toFixed(2) + '%');
  }
}
...
function updateMap(selectedImageName1, selectedImageName2, visParams) {
  ...
  // Update nitrogen legend visibility and labels
  updateNitrogenLegendVisibility(visParams1, visParams2);
