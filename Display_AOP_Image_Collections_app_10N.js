  ...
  // Immutable copy of the original defaults, used by the "Reset to Default" button below.
  // visParamsRGB itself is mutated in place by the RGB Display Settings panel so that
  // user-adjusted stretch/gamma values persist across image/collection changes
  // (updateMap() always reads the live visParamsRGB object for any RGB layer).
  var defaultRGBVisParams = {min: 40, max: 200, gamma: 0.65};
  ...
  function updateCustomBands() {
    ...
    // Keep the Reflectance Image Adjustments panel's fields in sync with the
    // newly selected custom bands' default stretch/gamma.
    syncReflectanceStretchControlsFromCurrentVisParams();
  ...
  var visParamsSelect = ui.Select({
    ...
    onChange: function(selectedOption) {
      ...
      // Keep the Reflectance Image Adjustments panel's fields in sync with the
      // newly selected preset's default stretch/gamma.
      syncReflectanceStretchControlsFromCurrentVisParams();
  ...
  // ----------------------------
  // Reflectance Image Adjustments Panel (Stretch / Gamma) - Dynamic
  // ----------------------------
  // Same in-app "gear icon" style stretch/gamma control as the RGB Camera panel,
  // generalized to whichever bands are currently active via currentVisParams.bands
  // (Natural Color, CIR, or Custom Band Selection). Applies to both the 1st and
  // 2nd image if both are Spectrometer Directional/Bidirectional Reflectance,
  // since currentVisParams already drives both slots identically. Embedded
  // directly in reflectanceFiltersPanel below the cloud filter dropdown, so its
  // visibility is governed by the same isSpecRefl check as the rest of that panel.

  var reflStretchPanelOpen = false;

  function isSpecReflCollection(col) {
    return col === 'Spectrometer Directional Reflectance (DP3.30006.001)' ||
           col === 'Spectrometer Bidirectional Reflectance (DP3.30006.002)';
  }

  // Returns whichever currently-displayed image (1st or 2nd) is a spectrometer
  // reflectance image, preferring the 1st image. Returns null if neither is.
  function getActiveReflectanceImage() {
    if (selectedImage1 && isSpecReflCollection(selectCollection1.getValue())) return selectedImage1;
    if (selectedImage2 && isSpecReflCollection(selectCollection2.getValue())) return selectedImage2;
    return null;
  }

  // Returns the true default min/max/gamma for whichever band combination is
  // currently selected, used by the "Reset to Default" button. Reads from the
  // visParamsOptions catalog (or the Custom Band Selection defaults) rather than
  // from currentVisParams itself, since currentVisParams may already hold a
  // user-applied override.
  function getDefaultReflectanceVisParams() {
    var selectedOption = visParamsSelect.getValue();
    if (selectedOption === 'Custom Band Selection') {
      return {min: 150, max: 3500, gamma: 1.0};
    }
    var preset = visParamsOptions[selectedOption];
    return {min: preset.min, max: preset.max, gamma: preset.gamma};
  }

  // Computes a shared min/max across all currently active bands for the given
  // stretch mode and applies it. Mirrors applyRGBStretchPreset but generalized
  // to an arbitrary band list (currentVisParams.bands).
  function applyReflectanceStretchPreset(mode) {
    var img = getActiveReflectanceImage();
    if (!img) {
      print('Reflectance Image Adjustments: select a Spectrometer Reflectance image first to use an auto-stretch preset.');
      return;
    }
    ...
    if (mode === '100%') {
      ...
      return; // 'Custom' - user enters Min/Max manually, nothing to compute
    }
    ...
    if (!lowVals.length || !highVals.length) {
      print('Reflectance Image Adjustments: could not compute stretch statistics for this image.');
      return;
    }
    ...
    reflMinBox.setValue(String(newMin));
    reflMaxBox.setValue(String(newMax));
    currentVisParams = {
      bands: currentVisParams.bands,
      min: newMin,
      max: newMax,
  ...
  var reflMinBox = ui.Textbox({
    value: String(currentVisParams.min),
    style: {width: '18%', margin: '2px 2% 2px 1%'}
  });
  var reflMaxBox = ui.Textbox({
    value: String(currentVisParams.max),
    style: {width: '18%', margin: '2px 2% 2px 0'}
  });
  var reflStretchModeSelect = ui.Select({
    items: ['Custom', '100%', '98%', '90%', '2 x Std Dev', '3 x Std Dev'],
    value: 'Custom',
    onChange: function(mode) { applyReflectanceStretchPreset(mode); },
    style: {stretch: 'horizontal', margin: '2px 1% 2px 0'}
  });
  var reflMinMaxLabels = ui.Panel({
    widgets: [
      ...
      ui.Label('Stretch', {stretch: 'horizontal', margin: '0 1% 0 0', fontSize: '11px', color: 'gray'})
    ],
    layout: ui.Panel.Layout.flow('horizontal'),
    style: {width: '100%'}
  });
  var reflMinMaxRow = ui.Panel({
    widgets: [reflMinBox, reflMaxBox, reflStretchModeSelect],
    layout: ui.Panel.Layout.flow('horizontal'),
    style: {width: '100%'}
  });

  var reflGammaLabel = ui.Label('Gamma: ' + currentVisParams.gamma.toFixed(2), {fontSize: '11px', margin: '4px 1%'});
  var reflGammaSlider = ui.Slider({
    min: 0.1,
    max: 3.0,
    value: currentVisParams.gamma,
    step: 0.05,
    style: {width: '100%', margin: '2px 1%'},
    onChange: function(value) {
      reflGammaLabel.setValue('Gamma: ' + value.toFixed(2));
      applyReflectanceStretch(true /* skip min/max re-read, already valid */);
    }
  });

  // Re-applies the current min/max/gamma to whichever reflectance layer(s) are
  // displayed. Rebuilds currentVisParams as a new object (rather than mutating
  // the shared visParamsOptions preset in place) so switching band combos still
  // starts from that preset's true defaults, while the override still persists
  // across 1st/2nd image changes (currentVisParams itself doesn't change unless
  // the user picks a different preset/custom bands).
  function applyReflectanceStretch(skipMinMaxParse) {
    var minVal = parseFloat(reflMinBox.getValue());
    var maxVal = parseFloat(reflMaxBox.getValue());
    if (!skipMinMaxParse) {
      if (isNaN(minVal) || isNaN(maxVal) || minVal >= maxVal) {
        print('Reflectance Image Adjustments: please enter valid Min/Max values (Min must be less than Max).');
        return;
      }
      reflStretchModeSelect.setValue('Custom', false);
    }
    currentVisParams = {
      bands: currentVisParams.bands,
      min: minVal,
      max: maxVal,
      gamma: reflGammaSlider.getValue()
    };
    updateMap(select1.getValue(), select2.getValue(), currentVisParams);
  }

  var reflApplyButton = ui.Button({
    label: 'Apply',
    onClick: function() { applyReflectanceStretch(false); },
    style: {width: '48%', margin: '4px 1%'}
  });

  var reflResetButton = ui.Button({
    label: 'Reset to default',
    onClick: function() {
      var def = getDefaultReflectanceVisParams();
      reflMinBox.setValue(String(def.min));
      reflMaxBox.setValue(String(def.max));
      reflGammaSlider.setValue(def.gamma);
      reflGammaLabel.setValue('Gamma: ' + def.gamma.toFixed(2));
      reflStretchModeSelect.setValue('Custom', false);
      currentVisParams = {
        bands: currentVisParams.bands,
        min: def.min,
  ...
  var reflButtonRow = ui.Panel({
    widgets: [reflApplyButton, reflResetButton],
    layout: ui.Panel.Layout.flow('horizontal'),
    style: {width: '100%'}
  });

  var reflStretchPanel = ui.Panel({
    widgets: [
      reflMinMaxLabels,
      reflMinMaxRow,
      reflGammaLabel,
  ...
  // Keeps the Min/Max/Gamma/Stretch-mode fields in sync with currentVisParams
  // whenever it changes for reasons other than this panel itself (preset
  // switch, custom band selection, or opening the panel).
  function syncReflectanceStretchControlsFromCurrentVisParams() {
    if (typeof reflMinBox === 'undefined') return; // not yet initialized
    reflMinBox.setValue(String(currentVisParams.min));
    reflMaxBox.setValue(String(currentVisParams.max));
    reflGammaSlider.setValue(currentVisParams.gamma);
    reflGammaLabel.setValue('Gamma: ' + currentVisParams.gamma.toFixed(2));
    reflStretchModeSelect.setValue('Custom', false);
  }

  // Combined into the existing "Reflectance Image Filters" section below the
  // cloud filter dropdown (no separate separator/title needed).
  var reflStretchToggleButton = ui.Button({
    label: 'Histogram Stretch / Gamma',
    onClick: function() {
      reflStretchPanelOpen = !reflStretchPanelOpen;
      if (reflStretchPanelOpen) { syncReflectanceStretchControlsFromCurrentVisParams(); }
      reflStretchPanel.style().set('shown', reflStretchPanelOpen);
    },
    style: {width: '100%', margin: '10px 0 0 0'}
  });
      ...
      ui.Panel({
        widgets: [
          ui.Panel([
            ...
            ui.Panel([
              ui.Label({
                value: 'Reflectance Image Filters / Adjustments',
                style: {fontSize: '12px', fontWeight: 'bold', color: '4A997E'}
              })
            ]),
            ui.Label({
  ...
  // ----------------------------
  // RGB Display Settings Panel (Stretch / Gamma) - Dynamic
  // ----------------------------
  // Provides an in-app equivalent of the "gear icon" layer visualization editor,
  // which GEE Apps hide from end users. Applies the same min/max/gamma stretch to
  // any RGB image(s) currently displayed as the 1st and/or 2nd image (they overlap
  // on the map rather than displaying side-by-side, so one shared stretch keeps the
  // panel compact instead of needing separate controls per image).

  var rgbStretchPanelOpen = false;

  // Returns whichever currently-displayed image (1st or 2nd) is from the RGB
  // Camera Photography collection, preferring the 1st image. Used as the basis
  // for the percentile/Std Dev auto-stretch presets below. Returns null if no
  // RGB image is currently displayed.
  function getActiveRGBImage() {
    var isRGB = function(col) { return col === 'RGB Camera Photography (DP3.30010.001)'; };
    if (selectedImage1 && isRGB(selectCollection1.getValue())) return selectedImage1;
    if (selectedImage2 && isRGB(selectCollection2.getValue())) return selectedImage2;
    return null;
  }

  // Computes a shared min/max across the R, G, and B bands for the given stretch
  // mode and applies it to the Min/Max textboxes + visParamsRGB. Mirrors the
  // percentile-clip stretch options in the GEE Code Editor's native layer
  // visualization ("gear icon") dialog.
  function applyRGBStretchPreset(mode) {
    var img = getActiveRGBImage();
    if (!img) {
      print('RGB Camera Image Adjustments: select an RGB image first to use an auto-stretch preset.');
      return;
    }
    ...
    var stats = img.select(['R', 'G', 'B']).reduceRegion({
      reducer: reducer,
      geometry: img.geometry(),
      scale: 20,
      bestEffort: true,
    ...
    if (!lowVals.length || !highVals.length) {
      print('RGB Camera Image Adjustments: could not compute stretch statistics for this image.');
      return;
    }
    ...
    rgbMinBox.setValue(String(newMin));
    rgbMaxBox.setValue(String(newMax));
    visParamsRGB.min = newMin;
    visParamsRGB.max = newMax;
    updateMap(select1.getValue(), select2.getValue(), currentVisParams);
  }

  var rgbMinBox = ui.Textbox({
    value: String(visParamsRGB.min),
    style: {width: '18%', margin: '2px 2% 2px 1%'}
  });
  var rgbMaxBox = ui.Textbox({
    value: String(visParamsRGB.max),
    style: {width: '18%', margin: '2px 2% 2px 0'}
  });
  var rgbStretchModeSelect = ui.Select({
    items: ['Custom', '100%', '98%', '90%', '2 x Std Dev', '3 x Std Dev'],
    value: 'Custom',
    onChange: function(mode) { applyRGBStretchPreset(mode); },
    style: {stretch: 'horizontal', margin: '2px 1% 2px 0'}
  });
  var rgbMinMaxLabels = ui.Panel({
    widgets: [
      ui.Label('Min', {width: '18%', margin: '0 2% 0 1%', fontSize: '11px', color: 'gray'}),
      ui.Label('Max', {width: '18%', margin: '0 2% 0 0', fontSize: '11px', color: 'gray'}),
      ui.Label('Stretch', {stretch: 'horizontal', margin: '0 1% 0 0', fontSize: '11px', color: 'gray'})
  ...
  var rgbMinMaxRow = ui.Panel({
    widgets: [rgbMinBox, rgbMaxBox, rgbStretchModeSelect],
    layout: ui.Panel.Layout.flow('horizontal'),
    style: {width: '100%'}
  });

  var rgbGammaLabel = ui.Label('Gamma: ' + visParamsRGB.gamma.toFixed(2), {fontSize: '11px', margin: '4px 1%'});
  var rgbGammaSlider = ui.Slider({
    min: 0.1,
    max: 3.0,
    value: visParamsRGB.gamma,
    step: 0.05,
    style: {width: '100%', margin: '2px 1%'},
    onChange: function(value) {
      visParamsRGB.gamma = value;
      rgbGammaLabel.setValue('Gamma: ' + value.toFixed(2));
      applyRGBStretch(true /* skip min/max re-read, already valid */);
    }
  });

  // Re-applies the current min/max/gamma to whichever RGB layer(s) are displayed.
  // Mutates visParamsRGB in place so the new values persist even if the user
  // subsequently changes the 1st or 2nd image/collection selection.
  function applyRGBStretch(skipMinMaxParse) {
    if (!skipMinMaxParse) {
      var minVal = parseFloat(rgbMinBox.getValue());
      var maxVal = parseFloat(rgbMaxBox.getValue());
      if (isNaN(minVal) || isNaN(maxVal) || minVal >= maxVal) {
        print('RGB Camera Image Adjustments: please enter valid Min/Max values (Min must be less than Max).');
        return;
      }
      visParamsRGB.min = minVal;
      visParamsRGB.max = maxVal;
      rgbStretchModeSelect.setValue('Custom', false); // manual edit - no longer a preset
    }
    updateMap(select1.getValue(), select2.getValue(), currentVisParams);
  }

  var rgbApplyButton = ui.Button({
    label: 'Apply',
    onClick: function() { applyRGBStretch(false); },
    style: {width: '48%', margin: '4px 1%'}
  });

  var rgbResetButton = ui.Button({
    label: 'Reset to default',
    onClick: function() {
      visParamsRGB.min = defaultRGBVisParams.min;
      visParamsRGB.max = defaultRGBVisParams.max;
      visParamsRGB.gamma = defaultRGBVisParams.gamma;
      rgbMinBox.setValue(String(defaultRGBVisParams.min));
      rgbMaxBox.setValue(String(defaultRGBVisParams.max));
      rgbGammaSlider.setValue(defaultRGBVisParams.gamma);
      rgbGammaLabel.setValue('Gamma: ' + defaultRGBVisParams.gamma.toFixed(2));
      rgbStretchModeSelect.setValue('Custom', false);
      updateMap(select1.getValue(), select2.getValue(), currentVisParams);
    },
    style: {width: '48%', margin: '4px 1%'}
  });

  var rgbButtonRow = ui.Panel({
    widgets: [rgbApplyButton, rgbResetButton],
    layout: ui.Panel.Layout.flow('horizontal'),
    style: {width: '100%'}
  });

  var rgbStretchPanel = ui.Panel({
    widgets: [
      rgbMinMaxLabels,
      rgbMinMaxRow,
      rgbGammaLabel,
  ...
  var rgbStretchToggleButton = ui.Button({
    label: 'Histogram Stretch / Gamma',
    onClick: function() {
      rgbStretchPanelOpen = !rgbStretchPanelOpen;
      rgbStretchPanel.style().set('shown', rgbStretchPanelOpen);
    },
    style: {width: '100%', margin: '4px 0 0 0', shown: false}
  });

  // Green line separator, matching the style used elsewhere (e.g. above the
  // Image Metadata section), to visually separate this panel from the
  // Reflectance Image Filters section above it.
  var rgbStretchSeparator = ui.Label({
    value: '_______________________________________________',
    style: {fontWeight: 'bold', color: '4A997E', shown: false}
  });

  // Centered title, matching the "Reflectance Image Filters" title pattern.
  var rgbStretchTitle = ui.Panel({
    widgets: [
      ui.Panel([
        ui.Label({value: ' ', style: {stretch: 'horizontal'}}),
        ui.Panel([
          ui.Label({
            value: 'RGB Camera Image Adjustments',
            style: {fontSize: '12px', fontWeight: 'bold', color: '4A997E'}
          })
        ]),
        ui.Label({value: ' ', style: {stretch: 'horizontal'}})
  ...
  mainPanel.add(rgbStretchSeparator);
  mainPanel.add(rgbStretchTitle);
  mainPanel.add(rgbStretchToggleButton);
  mainPanel.add(rgbStretchPanel);

  // Shows the toggle button only when the 1st and/or 2nd image is from the RGB
  // Camera Photography collection; collapses the panel when it's no longer relevant.
  function updateRGBStretchControlsVisibility() {
    var collection1 = selectCollection1.getValue();
    var collection2 = selectCollection2.getValue();
    var isRGB = function(col) { return col === 'RGB Camera Photography (DP3.30010.001)'; };
    var showControls = (collection1 && isRGB(collection1)) || (collection2 && isRGB(collection2));

    rgbStretchSeparator.style().set('shown', showControls);
    rgbStretchTitle.style().set('shown', showControls);
    rgbStretchToggleButton.style().set('shown', showControls);
    if (!showControls) {
      rgbStretchPanel.style().set('shown', false);
      rgbStretchPanelOpen = false;
    }
  }
  ...
  function updateFilterPanelVisibility() {
    ...
    // Show/hide the RGB stretch/gamma adjustment toggle
    updateRGBStretchControlsVisibility();
  }
  ...
  var chartTitle2 = ui.Label('Click on image(s) to view pixel values or spectra (export panel must be closed; refresh browser if spectral plot freezes)', {
    position: 'bottom-center',
    color: 'blue',
    fontSize: '13px',
    fontWeight: 'bold',
