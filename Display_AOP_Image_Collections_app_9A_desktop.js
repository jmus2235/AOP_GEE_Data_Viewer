...
function generateScriptContent(template, site, imageIndex, year, dynamicVisParams) {
  ...
  var scriptLines = [
    ...
    'var visParams = ' + visParamsToUse + ';',
    '',
    '// Add the layer to the map and center on the site',
    'Map.addLayer(selected_image, visParams, \'' + site + ' ' + year + ' ' + template.layerName + '\');',
...
// Add a horiontal line and then Reflectance Image Filters text
var specLine = ui.Panel([
  ui.Label({
...
mainPanel.add(specLine);

// Create a horizontal panel with flow layout to center the SpecReflectance Filter text
var ReflFilterHorizontalPanel1 = ui.Panel({
  layout: ui.Panel.Layout.flow('horizontal'),
  style: {
    stretch: 'horizontal'
  }
...
// Create flexible space elements to push the SpecReflectance Filte label to the center
var ReflFilterSpacerLeft = ui.Label({
  value: ' ',
  style: {stretch: 'horizontal'}
});

var specTitle = ui.Panel([
  ui.Label({
    value: 'Reflectance Image Filters',
    //style: {textDecoration: 'underline', fontSize: '12px', fontWeight: 'bold',  color: '4A997E'},
    style: {fontSize: '12px', fontWeight: 'bold',  color: '4A997E'},
  }),
  ]);
//mainPanel.add(specTitle);

var ReflFilterSpacerRight = ui.Label({
  value: ' ',
  style: {stretch: 'horizontal'}
});

ReflFilterHorizontalPanel1.add(ReflFilterSpacerLeft);
ReflFilterHorizontalPanel1.add(specTitle);
ReflFilterHorizontalPanel1.add(ReflFilterSpacerRight);
mainPanel.add(ReflFilterHorizontalPanel1);
...
mainPanel.add(visParamsSelect);

mainPanel.add(customBandsPanel);
...
mainPanel.add(cloudFilterSelect);
