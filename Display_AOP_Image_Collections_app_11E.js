    ...
    // Region-statistics tool (1st image only) â€” shares the single Map.drawingTools()
    // instance with the crop/export tool, so only one of the two can be active at a time.
    var statsPanelOpen = false;
    var drawingToolMode = 'crop';  // 'crop' | 'stats' â€” routes onDraw/onEdit to the right handler
    var statsRegionGeometry = null;
    var statsRegionLayer = null;
    ...
    // Set up drawing event handlers â€” routed by drawingToolMode since the crop/export
    // tool and the region-stats tool share this single drawingTools instance.
    drawingTools.onDraw(function() {
      if (drawingToolMode === 'stats') {
        updateStatsGeometry();
      } else {
        updateCropGeometry();
      }
    ...
    // Region-statistics panel â€” floats on the map, bottom-left. Populated live
    // as the user draws or edits a rectangle/polygon over the 1st image.
    var statsProductLabel = ui.Label({value: '', style: {fontWeight: 'bold', fontSize: '12px', margin: '4px 4px 6px 4px'}});
    var statsMinLabel      = ui.Label('Min: —',      {fontSize: '11px', margin: '2px 4px'});
    var statsMaxLabel      = ui.Label('Max: —',      {fontSize: '11px', margin: '2px 4px'});
    var statsPct98MaxLabel = ui.Label('98% Max: —',  {fontSize: '11px', margin: '2px 4px'});
    var statsMeanLabel     = ui.Label('Mean: —',     {fontSize: '11px', margin: '2px 4px'});
    var statsMedianLabel   = ui.Label('Median: —',   {fontSize: '11px', margin: '2px 4px'});
    var statsStdDevLabel   = ui.Label('StDev: —',    {fontSize: '11px', margin: '2px 4px'});
    var statsPixelCountLabel = ui.Label({value: '', style: {fontSize: '10px', fontStyle: 'italic', color: 'gray', margin: '4px 4px 0 4px'}});
    ...
    var statsPanel = ui.Panel({
      widgets: [
        ...
        ui.Panel({
          widgets: [statsMeanLabel, statsMedianLabel, statsStdDevLabel],
          layout: ui.Panel.Layout.flow('horizontal'),
          style: {width: '100%'}
        }),
        statsPixelCountLabel,
        ui.Panel({
          widgets: [
            ui.Button({label: 'Clear Region', onClick: clearStatsRegion, style: {width: '48%', margin: '5px 1% 0 1%'}}),
            ui.Button({label: 'Close', onClick: closeStatsPanel,
                      style: {width: '48%', margin: '5px 1% 0 1%', backgroundColor: '#ff9999'}})
          ],
          layout: ui.Panel.Layout.flow('horizontal'),
          style: {width: '100%'}
        })
    ...
    function openExportPanel() {
      ...
      if (statsPanelOpen) {
        print('Please close the "1st Image Region Statistics" panel before opening the Export panel.');
        return;
      }

      exportPanel.style().set('shown', true);
      exportPanelOpen = true;
      drawingToolMode = 'crop';
      ...
      drawingTools.setDrawModes(['rectangle']); // Crop tool is rectangle-only
    ...
    // ----------------------------
    // Draw and Generate Stats (1st image only)
    // ----------------------------

    // Single-band collections where the currently selected 1st image is stats-eligible
    // as-is (no per-product compute pipeline needed).
    var STATS_RAW_BAND_META = {
      CHM: {label: 'CHM', unit: ' m', decimals: 1},
      DSM: {label: 'DSM', unit: ' m', decimals: 1},
      DTM: {label: 'DTM', unit: ' m', decimals: 1},
      Nitrogen_Percent: {label: 'Canopy Nitrogen', unit: ' %', decimals: 2},
      Nitrogen_Uncertainty: {label: 'Nitrogen Uncertainty', unit: ' %', decimals: 3}
    };

    // Returns {eligible: false} or {eligible: true, mode: 'raw'|'derived', ...} for
    // the currently selected 1st image/product. Veg/biophysical/water indices,
    // CHM, DSM, DTM, and Canopy Nitrogen are eligible; Albedo and terrain products
    // (Slope/Aspect/Hillshade) are excluded.
    function getStatsEligibility() {
      var col1 = selectCollection1.getValue();
      if (!col1 || !select1.getValue()) return {eligible: false};

      if (isCHMCollection(col1)) {
        var chmM = STATS_RAW_BAND_META.CHM;
        return {eligible: true, mode: 'raw', band: 'CHM', label: chmM.label, unit: chmM.unit, decimals: chmM.decimals};
      }
      if (isDSMCollection(col1)) {
        var dsmM = STATS_RAW_BAND_META.DSM;
        return {eligible: true, mode: 'raw', band: 'DSM', label: dsmM.label, unit: dsmM.unit, decimals: dsmM.decimals};
      }
      if (isDTMCollection(col1)) {
        var dtmM = STATS_RAW_BAND_META.DTM;
        return {eligible: true, mode: 'raw', band: 'DTM', label: dtmM.label, unit: dtmM.unit, decimals: dtmM.decimals};
      }
      if (isNitrogenCollection(col1)) {
        // Eligibility (and which band gets reduced) must follow the currently
        // selected nitrogen band for the 1st image slot, not always Nitrogen_Percent
        // — otherwise stats silently show Percent Nitrogen values while the label/
        // dropdown claims Uncertainty or Classification is active.
        var nBand1 = getEffectiveNitrogenBand(1);
        if (nBand1 === 'Canopy Nitrogen Model Uncertainty') {
          var nuM = STATS_RAW_BAND_META.Nitrogen_Uncertainty;
          return {eligible: true, mode: 'raw', band: 'Nitrogen_Uncertainty', label: nuM.label, unit: nuM.unit, decimals: nuM.decimals};
        }
        if (nBand1 === 'Needle Leaf/Non-Needle Leaf Classification') {
          // Classification is categorical (Needle/Non-Needle) — Min/Mean/Max/StDev
          // don't apply, so treat as ineligible rather than showing bogus numbers.
          return {eligible: false};
        }
        var nM = STATS_RAW_BAND_META.Nitrogen_Percent;
        return {eligible: true, mode: 'raw', band: 'Nitrogen_Percent', label: nM.label, unit: nM.unit, decimals: nM.decimals};
      }

      if (isDerivedIndicesCollection(col1)) {
        var eligibleIndexProducts = viProductNames.concat(viProductNames2).concat(wiProductNames);
        if (eligibleIndexProducts.indexOf(currentDerivedProduct) !== -1) {
          var dm = DERIVED_LEGEND_META[currentDerivedProduct];
          return {eligible: true, mode: 'derived', productName: currentDerivedProduct,
                  label: currentDerivedProduct, unit: dm.unit, decimals: dm.decimals};
        }
      }
      return {eligible: false};
    }

    // Builds the single-band ee.Image to reduce, mirroring addDerivedProductLayer's
    // per-product dispatch so stats match exactly what's rendered on the map.
    function computeStatsImage(elig) {
      if (elig.mode === 'raw') {
        return {image: selectedImage1.select(elig.band), bandName: elig.band};
      }
      var b = vi_precompute(selectedImage1);
      var p = elig.productName;
      var result;
      if      (p === 'NDVI') result = vi_ndvi(b);
      else if (p === 'EVI')  result = vi_evi(b);
      else if (p === 'ARVI') result = vi_arvi(b);
      else if (p === 'PRI')  result = vi_pri(b);
      else if (p === 'SAVI') result = vi_savi(b);
      else if (p === 'LAI')  result = vi_lai(b);
      else if (p === 'fPAR') result = vi_fpar(b);
      else if (p === 'WBI')  result = wi_wbi(selectedImage1,  b.water);
      else if (p === 'NMDI') result = wi_nmdi(selectedImage1, b.water);
      else if (p === 'NDWI') result = wi_ndwi(selectedImage1, b.water);
      else if (p === 'NDII') result = wi_ndii(selectedImage1, b.water);
      else if (p === 'MSI')  result = wi_msi(selectedImage1,  b.water);
      return {image: result, bandName: p};
    }

    // Opens the region-stats panel and activates the shared drawing tool in
    // rectangle+polygon mode. Requires the Export panel to be closed first since
    // both tools drive the same Map.drawingTools() singleton.
    function openStatsPanel() {
      var elig = getStatsEligibility();
      if (!elig.eligible) {
        print('Region statistics are only available for CHM, DSM, DTM, Canopy Nitrogen, or a veg/biophysical/water index on the 1st image.');
        return;
      }
      if (exportPanelOpen) {
        print('Please close the Export panel before drawing a stats region.');
        return;
      }

      statsPanelOpen = true;
      drawingToolMode = 'stats';
      statsPanel.style().set('shown', true);
      statsProductLabel.setValue(elig.label);
      statsMinLabel.setValue('Min: —');
      statsMaxLabel.setValue('Max: —');
      statsPct98MaxLabel.setValue('98% Max: —');
      statsMeanLabel.setValue('Mean: —');
      statsMedianLabel.setValue('Median: —');
      statsStdDevLabel.setValue('StDev: —');
      statsPixelCountLabel.setValue('');
    ...
    // Closes the region-stats panel, stops drawing, and restores the shared
    // drawing tool to the crop tool's rectangle-only default.
    function closeStatsPanel() {
      ...
      statsRegionGeometry = null;
      if (statsRegionLayer) {
        Map.remove(statsRegionLayer);
        statsRegionLayer = null;
      }
    }

    // Clears the drawn region so the user can redraw without closing the panel.
    function clearStatsRegion() {
      drawingTools.layers().reset();
      statsRegionGeometry = null;
      if (statsRegionLayer) {
        Map.remove(statsRegionLayer);
    ...
    // Called on every onDraw/onEdit while the stats tool is active.
    function updateStatsGeometry() {
      var layers = drawingTools.layers();
      if (layers.length() === 0) return;

      statsRegionGeometry = layers.get(0).toGeometry();

      if (statsRegionLayer) {
        Map.remove(statsRegionLayer);
      }
      statsRegionLayer = ui.Map.Layer(
        ee.FeatureCollection([ee.Feature(statsRegionGeometry)]).style({
          color: 'purple',
          fillColor: '00000000',
          width: 3
        }),
      ...
      Map.add(statsRegionLayer);

      computeAndDisplayStats();
    }

    // Reduces the drawn region over the current single-band 1st-image product and
    // updates the stats panel. bestEffort/tileScale keep large regions responsive
    // by coarsening the effective scale rather than erroring out; if this proves
    // too slow in practice for large polygons, gate this call behind a manual
    // "Calculate Stats" button instead of calling it live from onDraw/onEdit.
    function computeAndDisplayStats() {
      var elig = getStatsEligibility();
      if (!elig.eligible || !statsRegionGeometry) return;

      statsMinLabel.setValue('Min: calculating…');
      statsMaxLabel.setValue('Max: calculating…');
      statsPct98MaxLabel.setValue('98% Max: calculating…');
      statsMeanLabel.setValue('Mean: calculating…');
      statsMedianLabel.setValue('Median: calculating…');
      statsStdDevLabel.setValue('StDev: calculating…');

      var imgInfo = computeStatsImage(elig);
      ...
      var combined = ee.Dictionary({
        stats: imgInfo.image.reduceRegion({
          reducer: reducer,
          geometry: statsRegionGeometry,
          scale: 10,
          bestEffort: true,
          maxPixels: 1e9,
          tileScale: 4
        }),
        areaM2: statsRegionGeometry.area(1)
      });

      combined.evaluate(function(result) {
        var stats = result && result.stats;
        if (!stats) return;
        var band = imgInfo.bandName;
        var mn = stats[band + '_min'], mean = stats[band + '_mean'], median = stats[band + '_median'],
            mx = stats[band + '_max'], sd = stats[band + '_stdDev'],
            n  = stats[band + '_count'];
        var dec = elig.decimals, unit = elig.unit;

        statsMinLabel.setValue('Min: '      + (mn     !== null && mn     !== undefined ? mn.toFixed(dec)     + unit : 'masked'));
        statsMaxLabel.setValue('Max: '      + (mx     !== null && mx     !== undefined ? mx.toFixed(dec)     + unit : 'masked'));
        statsPct98MaxLabel.setValue('98% Max: ' + (mx  !== null && mx     !== undefined ? (mx * 0.98).toFixed(dec) + unit : 'masked'));
        statsMeanLabel.setValue('Mean: '    + (mean   !== null && mean   !== undefined ? mean.toFixed(dec)   + unit : 'masked'));
        statsMedianLabel.setValue('Median: ' + (median !== null && median !== undefined ? median.toFixed(dec) + unit : 'masked'));
    ...
    // Enables/disables the stats button to match the currently selected 1st
    // image/product, and keeps an already-open panel in sync with that selection.
    function updateStatsButtonState() {
      var elig = getStatsEligibility();
      statsButton.setDisabled(!elig.eligible);
      if (!elig.eligible) {
      ...
      if (statsPanelOpen) {
        statsProductLabel.setValue(elig.label);
        if (statsRegionGeometry) computeAndDisplayStats();
      }
    }
    ...
    // Note: this still shares the right-hand map column with the bottom-right
    // legend panels (derived indices/nitrogen), so on short screens they can
    // still compete for vertical space if both are shown simultaneously.
    ...
    // object. Min/max/gamma/palette are overridden; bands are left untouched.
    function applyManualOverride(slot, natural) {
      ...
      if (ov && key && ov.key === key) {
        ...
        // Palette swap is only meaningful for palette-based types, and is a pure
        // visParams substitution (no image transform needed, unlike a color-space
        // adjustment) - see slotSupportsPalette/PALETTE_CHOICES_BY_PRODUCT.
        if (natural.palette !== undefined && ov.palette !== undefined) { merged.palette = ov.palette; }
        return merged;
      }
      return natural;
    }
    ...
    var nitrogenBandSelect1 = ui.Select({
      ...
      onChange: function(selectedOption) {
        ...
        updateStatsButtonState(); // Keep an already-open stats panel/button in sync with the new band
      },
    ...
    // Same staleness-tracking pattern as the Stretch preset, but for the
    // Palette dropdown: null means "use this sub-type's natural default"
    // (PALETTE_CHOICES_BY_PRODUCT[key][0]); a switch to a different sub-type/
    // image in this slot (different override key) invalidates a stale pick.
    var universalPaletteLabelBySlot = {1: null, 2: null};
    var universalPaletteKeyBySlot = {1: null, 2: null};
    ...
    // ----------------------------
    // Palette switching - state & helpers
    // ----------------------------
    // Palette swap is a pure visParams field (like Min/Max), merged in
    // applyManualOverride with no image transform - unlike a color-space
    // adjustment, it costs nothing extra to render. Only offered for
    // palette-based (single-band) products: CHM/DSM/DTM/Nitrogen (Percent and
    // Uncertainty, not the categorical Classification band)/veg+water indices/
    // Slope. Excluded: Reflectance/RGB (no palette to swap), Albedo (already
    // compute-heavy, palette choice adds little), Nitrogen Classification and
    // Aspect (fixed categorical/cyclic palettes tied to their legends), and
    // Hillshade (grayscale is the point of a relief-shading product).
    var PALETTE_LIBRARY = {
      'Green Ramp': ['E6F7E0', '063B00'],
      'Grayscale': ['000000', 'FFFFFF'],
      'Viridis': ['440154', '3b528b', '21908c', '5dc963', 'fde725'],
      'Plasma': ['0d0887', '7e03a8', 'cc4778', 'f89540', 'f0f921'],
      'Heat': ['ffffb2', 'fecc5c', 'fd8d3c', 'f03b20', 'bd0026'],
      'Hypsometric': ['1a9850', 'a6d96a', 'fee08b', '8c510a', 'ffffff'],
      'Hypsometric (no white)': ['1a9850', 'a6d96a', 'fee08b', '8c510a'],
      'Brown Ramp': ['ffffff', '8b4513'],
      'Red-Yellow-Green': ['d73027', 'f46d43', 'fdae61', 'fee08b', 'd9ef8b', 'a6d96a', '66bd63', '1a9850'],
      'Yellow-Green-Teal': ['d73027', 'f46d43', 'fdae61', 'fee08b', 'd9ef8b', 'a6d96a', '1a9850', '00b09b'],
      'Brown-Teal': ['8c510a', 'bf812d', 'dfc27d', 'f5f5f5', '80cdc1', '35978f', '01665e'],
      'Teal-Brown': ['01665e', '35978f', '80cdc1', 'f5f5f5', 'dfc27d', 'bf812d', '8c510a'],
      'Blue-Red': ['2166ac', '67a9cf', 'd1e5f0', 'f7f7f7', 'fddbc7', 'ef8a62', 'b2182b']
    };

    // Ordered [defaultName, ...alternateNames] per eligible product/sub-type.
    // The first name is always whatever's already showing by default for that
    // product - no separate "Default" entry needed, since that IS the default.
    var PALETTE_CHOICES_BY_PRODUCT = {
      CHM:                   ['Green Ramp', 'Viridis', 'Grayscale', 'Heat'],
      DSM:                   ['Grayscale', 'Viridis', 'Hypsometric', 'Hypsometric (no white)'],
      DTM:                   ['Grayscale', 'Viridis', 'Hypsometric', 'Hypsometric (no white)'],
      Nitrogen_Percent:      ['Viridis', 'Plasma', 'Green Ramp', 'Grayscale'],
      Nitrogen_Uncertainty:  ['Plasma', 'Viridis', 'Heat', 'Grayscale'],
      Slope:                 ['Brown Ramp', 'Viridis', 'Grayscale', 'Heat'],
      NDVI:                  ['Red-Yellow-Green', 'Brown-Teal', 'Blue-Red', 'Viridis'],
      EVI:                   ['Red-Yellow-Green', 'Brown-Teal', 'Blue-Red', 'Viridis'],
      ARVI:                  ['Red-Yellow-Green', 'Brown-Teal', 'Blue-Red', 'Viridis'],
      SAVI:                  ['Red-Yellow-Green', 'Brown-Teal', 'Blue-Red', 'Viridis'],
      LAI:                   ['Red-Yellow-Green', 'Brown-Teal', 'Blue-Red', 'Viridis'],
      fPAR:                  ['Yellow-Green-Teal', 'Red-Yellow-Green', 'Brown-Teal', 'Blue-Red'],
      PRI:                   ['Brown-Teal', 'Red-Yellow-Green', 'Blue-Red', 'Viridis'],
      WBI:                   ['Brown-Teal', 'Teal-Brown', 'Blue-Red', 'Red-Yellow-Green'],
      NDWI:                  ['Brown-Teal', 'Teal-Brown', 'Blue-Red', 'Red-Yellow-Green'],
      NDII:                  ['Brown-Teal', 'Teal-Brown', 'Blue-Red', 'Red-Yellow-Green'],
      NMDI:                  ['Teal-Brown', 'Brown-Teal', 'Blue-Red', 'Red-Yellow-Green'],
      MSI:                   ['Teal-Brown', 'Brown-Teal', 'Blue-Red', 'Red-Yellow-Green']
    };

    // Resolves which PALETTE_CHOICES_BY_PRODUCT key (if any) applies to whatever
    // is currently active in a slot. Returns null for ineligible types/sub-types.
    function getSlotPaletteProductKey(slot) {
      var col = (slot === 1) ? selectCollection1.getValue() : selectCollection2.getValue();
      if (!col) return null;
      if (isCHMCollection(col)) return 'CHM';
      if (isDSMCollection(col)) return 'DSM';
      ...
      if (isNitrogenCollection(col)) {
        var band = getEffectiveNitrogenBand(slot);
        if (band === 'Percent Nitrogen (%)') return 'Nitrogen_Percent';
        if (band === 'Canopy Nitrogen Model Uncertainty') return 'Nitrogen_Uncertainty';
        return null; // Needle Leaf/Non-Needle Leaf Classification - categorical, excluded
      }
      if (isDerivedIndicesCollection(col)) {
        return PALETTE_CHOICES_BY_PRODUCT.hasOwnProperty(currentDerivedProduct) ? currentDerivedProduct : null; // excludes Albedo
      }
      if (isDerivedTerrainCollection(col)) {
        return (currentTerrainProduct === 'Slope') ? 'Slope' : null; // Aspect/Hillshade excluded
      }
      return null; // Reflectance/RGB - no palette to swap
    }

    function slotSupportsPalette(slot) {
      return getSlotPaletteProductKey(slot) !== null;
    }

    function getSlotPaletteNames(slot) {
      var key = getSlotPaletteProductKey(slot);
      return key ? PALETTE_CHOICES_BY_PRODUCT[key] : null;
    }
    ...
    function applyUniversalStretchPreset(mode) {
      ...
      } else if (mode === '2σ' || mode === '3σ') {
        reducer = ee.Reducer.mean().combine({reducer2: ee.Reducer.stdDev(), sharedInputs: true});
        stdDevMultiplier = (mode === '2σ') ? 2 : 3;
      } else {
        return;
      }
    ...
    var universalMinBox = ui.Textbox({
      value: '0',
      style: {width: '14%', margin: '2px 1% 2px 1%'}
    });
    var universalMaxBox = ui.Textbox({
      value: '1',
      style: {width: '14%', margin: '2px 1% 2px 0'}
    });
    var universalStretchModeSelect = ui.Select({
      items: ['Custom', '100%', '98%', '90%', '2σ', '3σ'],
      value: 'Custom',
      onChange: function(mode) { applyUniversalStretchPreset(mode); },
      style: {width: '25%', margin: '2px 1% 2px 0'}
    });
    // Palette dropdown - only enabled for palette-based products (see
    // slotSupportsPalette). Selecting an entry applies immediately, same as
    // the Stretch preset dropdown, rather than requiring the Apply button.
    var universalPaletteSelect = ui.Select({
      items: ['Palette'],
      placeholder: 'Palette',
      onChange: function(paletteName) {
        var slot = universalAdjustSlot;
        universalPaletteLabelBySlot[slot] = paletteName;
        universalPaletteKeyBySlot[slot] = getOverrideKey(slot);
        applyUniversalStretch(true /* skip min/max re-read, already valid */);
      },
      style: {width: '36%', margin: '2px 0 2px 0'}
    });
    var universalStretchModeLabel = ui.Label('Stretch', {width: '22%', margin: '0 1% 0 0', fontSize: '11px', color: 'gray'});
    var universalPaletteLabel = ui.Label('Palette', {width: '36%', margin: '0 0 0 10px', fontSize: '11px', color: 'gray'});
    var universalMinMaxLabels = ui.Panel({
      widgets: [
        ui.Label('Min', {width: '14%', margin: '0 1% 0 1%', fontSize: '11px', color: 'gray'}),
        ui.Label('Max', {width: '14%', margin: '0 1% 0 0', fontSize: '11px', color: 'gray'}),
        universalStretchModeLabel,
        universalPaletteLabel
      ],
      layout: ui.Panel.Layout.flow('horizontal'),
      style: {width: '100%'}
    });
    var universalMinMaxRow = ui.Panel({
      widgets: [universalMinBox, universalMaxBox, universalStretchModeSelect, universalPaletteSelect],
      layout: ui.Panel.Layout.flow('horizontal'),
      style: {width: '100%'}
    });
    ...
    function applyUniversalStretch(skipMinMaxParse) {
      ...
      // Resolve active palette: use the per-slot recorded choice if it's still
      // valid for this slot's current sub-type; otherwise fall back to the
      // sub-type's natural default (first entry in PALETTE_CHOICES_BY_PRODUCT).
      var activePaletteName = null;
      if (slotSupportsPalette(slot)) {
        var pKey = getOverrideKey(slot);
        var pLabel = universalPaletteLabelBySlot[slot];
        if (pLabel && universalPaletteKeyBySlot[slot] === pKey) {
          activePaletteName = pLabel;
        } else {
          var choices = getSlotPaletteNames(slot);
          activePaletteName = choices ? choices[0] : null;
        }
      }
      manualOverrides[slot] = {
        ...
        palette: activePaletteName ? PALETTE_LIBRARY[activePaletteName] : undefined
      };
      updateMap(select1.getValue(), select2.getValue(), currentVisParams);
    }
    ...
    var universalResetButton = ui.Button({
      ...
      onClick: function() {
        ...
        universalStretchModeSelect.setValue('Custom', false);
        universalPaletteLabelBySlot[universalAdjustSlot] = null;
        universalPaletteKeyBySlot[universalAdjustSlot] = null;
        updateMap(select1.getValue(), select2.getValue(), currentVisParams);
      },
      style: {width: '38%', margin: '4px 1%'}
    });

    // Closes the panel the same way clicking the "Histogram Stretch / Gamma"
    // toggle button while open would - provided as an explicit button too since
    // users may not realize the toggle button also closes it.
    function closeUniversalStretchPanel() {
      universalStretchPanelOpen = false;
      universalStretchPanel.style().set('shown', false);
    }

    var universalCloseButton = ui.Button({
      label: 'Close',
      onClick: closeUniversalStretchPanel,
      style: {width: '26%', margin: '4px 1%', backgroundColor: '#ff9999'}
    });

    var universalButtonRow = ui.Panel({
      widgets: [universalApplyButton, universalResetButton, universalCloseButton],
      layout: ui.Panel.Layout.flow('horizontal'),
      style: {width: '100%'}
    });
    ...
    var universalStretchToggleButton = ui.Button({
      label: 'Histogram Stretch / Gamma / Palette',
      onClick: function() {
    ...
    function refreshUniversalPanelFields() {
      ...
      // Sync palette dropdown
      var paletteOk = slotSupportsPalette(slot);
      universalPaletteSelect.setDisabled(!paletteOk);
      if (paletteOk) {
        var choices = getSlotPaletteNames(slot);
        universalPaletteSelect.items().reset(choices);
        // Determine which palette is currently active for this slot
        var pKey = getOverrideKey(slot);
        var currentPaletteLabel = universalPaletteLabelBySlot[slot];
        if (!currentPaletteLabel || universalPaletteKeyBySlot[slot] !== pKey) {
          // Sub-type changed or no selection yet - default to the first choice
          currentPaletteLabel = choices[0];
        }
        universalPaletteSelect.setValue(currentPaletteLabel, false);
        universalPaletteLabel.setValue('Palette');
      } else {
        universalPaletteSelect.items().reset(['Palette']);
        universalPaletteLabel.setValue('Palette: N/A');
      }
    }
    ...
    // Region-stats panel stays in the bottom-left corner (with the legend panels);
    // the chart/pixel-info panels below are in the bottom-right corner instead.
    Map.add(statsPanel);
    ...
    Map.onClick(function(coords) {
      ...
      if (isEligibleCollection1) {
        // Drop the last 10 bands - the SWIR tail is noisy and can spike the chart for some images.
        var bandNames = currentImage1.bandNames().filter(ee.Filter.stringContains('item', 'B')).slice(0, -10);
        var properties = currentImage1.toDictionary();
        var wl_fwhm_dict = properties.select(['WL_FWHM_B+\\d{3}']);
        var wl_fwhm_list = wl_fwhm_dict.values().slice(0, -10);
      ...
      if (isEligibleCollection2 && currentImage2) {
        // Drop the last 10 bands - the SWIR tail is noisy and can spike the chart for some images.
        var bandNames2 = currentImage2.bandNames().filter(ee.Filter.stringContains('item', 'B')).slice(0, -10);
        var properties2 = currentImage2.toDictionary();
        var wl_fwhm_dict2 = properties2.select(['WL_FWHM_B+\\d{3}']);
        var wl_fwhm_list2 = wl_fwhm_dict2.values().slice(0, -10);
