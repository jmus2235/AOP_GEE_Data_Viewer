...
var scriptTemplates = {
  ...
  "Canopy Nitrogen Concentration (CNC/002)": {
    ...
    collectionName: "CNC",
    bands: "['nitrogen']",
    visParams: "{bands: ['nitrogen'], min: 0, max: 4, palette: ['darkblue', 'blue', 'cyan', 'yellow', 'orange', 'white']}",
  ...
  if (selectedCollection === "Digital Surface Model (DEM/001)") {
    ...
    selectedImage.get('FLIGHT_YEAR').evaluate(function(year) {
      ...
      if (selectedBand === 'Canopy Nitrogen Model Uncertainty') {
        visParamsString = '{bands: [\'nitrogen_uncertainty\'], min: 0, max: 1, palette: [\'darkblue\', \'blue\', \'cyan\', \'yellow\', \'orange\', \'red\']}';
      } else if (selectedBand === 'Needle Leaf/Non-Needle Leaf Classification') {
        visParamsString = '{bands: [\'nitrogen_classification\'], min: 0, max: 1, palette: [\'olive\', \'green\']}';
      } else {
        visParamsString = '{bands: [\'nitrogen\'], min: ' + dynamicParams.min.toFixed(2) + ', max: ' + dynamicParams.max.toFixed(2) + ', palette: [\'darkblue\', \'blue\', \'cyan\', \'yellow\', \'orange\', \'white\']}';
      }
...
function generateScriptContent(template, site, imageIndex, year, dynamicVisParams) {
  ...
  if (template.collectionName === 'CNC') {
    scriptLines.push('');
    scriptLines.push('// Mask nitrogen band using nitrogen_valid band');
    scriptLines.push('var nit_valid = selected_image.select(\'nitrogen_valid\');');
...
function exportCroppedImage() {
  ...
  exportGeometry.bounds().coordinates().evaluate(function(coords) {
    ...
    var collectionName = selectCollection1.getValue().split(' ')[0];
...
var percentNPalette = ['darkblue', 'blue', 'cyan', 'yellow', 'orange', 'white'];
...
var uncertaintyPalette = ['darkblue', 'blue', 'cyan', 'yellow', 'orange', 'red'];
...
var visParamsNitrogen = {bands: ['nitrogen'], min: 0, max: 4, palette: ['darkblue', 'blue', 'cyan', 'yellow', 'orange', 'white']};
...
function updateFilterPanelVisibility() {
  ...
  // Show reflectance filters if either image is HSI_REFL
  var showReflectance = (collection1 && String(collection1).indexOf('HSI_REFL') !== -1) || 
                        (collection2 && String(collection2).indexOf('HSI_REFL') !== -1);
...
function getDynamicNitrogenVisParams(nitrogenImage, imageNumber) {
  ...
  var maskedNitrogen = maskNitrogen(nitrogenImage).select('nitrogen');
  ...
  return {
    ...
    min: minVal,
    max: maxVal,
    palette: ['darkblue', 'blue', 'cyan', 'yellow', 'orange', 'white']
  };
}

function getNitrogenUncertaintyVisParams(nitrogenImage, imageNumber) {
  // Calculate actual min/max for uncertainty band
  var maskedUncertainty = maskNitrogen(nitrogenImage).select('nitrogen_uncertainty');
  
  var uncertaintyMinMax_local = maskedUncertainty.reduceRegion({
    reducer: ee.Reducer.minMax(),
    scale: 10,
    maxPixels: 3e7
  });
  
  var minVal = ee.Number(uncertaintyMinMax_local.get('nitrogen_uncertainty_min')).getInfo();
  var maxVal = ee.Number(uncertaintyMinMax_local.get('nitrogen_uncertainty_max')).getInfo();
...
// Function to mask nitrogen using the nitrogen_valid band
function maskNitrogen(image) {
  var nit_valid = image.select('nitrogen_valid');
