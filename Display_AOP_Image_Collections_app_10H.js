  ...
  // NLCD layer object — only added to the map when the checkbox is checked,
  ...
  // Full names for alias sites — used in generated sample script comments
  var pairedSiteNames = {
  ...
  var select1 = ui.Select({
    ...
    onChange: function(selectedImageName) {
      // When a Derived Indices image is selected, check whether a same-year DEM exists
      if (selectedImageName &&
          selectCollection1.getValue() === 'Derived Indices (HSI_REFL/002) / Terrain Products (DEM/001)') {
        var hsiImg = imageCollections[selectCollection1.getValue()]
          .filter(ee.Filter.eq('system:index', selectedImageName)).first();
        checkDemAvailability(hsiImg);
      }
  ...
  var select2 = ui.Select({
    ...
    onChange: function(selectedImageName) {
      // When a Derived Indices image is selected as the 2nd image, check whether a same-year DEM exists
      if (selectedImageName &&
          selectCollection2.getValue() === 'Derived Indices (HSI_REFL/002) / Terrain Products (DEM/001)') {
        var hsiImg = imageCollections[selectCollection2.getValue()]
          .filter(ee.Filter.eq('system:index', selectedImageName)).first();
        checkDemAvailability(hsiImg);
      }
  ...
  function generateSampleScript() {
    ...
    if (selectedCollection === 'Derived Indices (HSI_REFL/002) / Terrain Products (DEM/001)') {
      ...
      derivedImage.get('FLIGHT_YEAR').evaluate(function(year) {
        ...
        if (isTerrainProduct) {
          lines = lines.concat([
            '// Read in the NEON AOP Bidirectional Reflectance Image Collection (HSI_REFL/002)',
            '// to get the site footprint geometry for spatial filtering of the DEM tiles',
            ...
            '',
            '// Read in the NEON AOP Digital Elevation Model Image Collection (DEM/001)',
            '// Load DEM tiles intersecting the site footprint and compute terrain products per tile,',
            '// then mosaic — preserves native UTM projection so terrain derivatives are in meters.',
  ...
  function exportCroppedImage() {
    ...
    if (isDerivedExport) {
      ...
      if (terrainBandMapExport.hasOwnProperty(currentDerivedProduct)) {
        ...
        else if (currentDerivedProduct === 'LAI')  selectedImage1 = vi_lai(viBands);
        else if (currentDerivedProduct === 'fPAR') selectedImage1 = vi_fpar(viBands);
      }
    }
  ...
  // Persistent references to the currently active derived legend min/max labels.
  // Reassigned inside createDerivedLegend each time so the async evaluate()
  // callback always updates the correct (most recently created) label widgets.
  var derivedMinLabel = null;
  var derivedMaxLabel = null;
  ...
  function createDerivedLegend(productName) {
    ...
    // Create fresh label widgets each call to avoid GEE widget re-parenting issues.
    // The module-level references are updated so the async evaluate() callback
    // can update whichever labels are currently visible.
    derivedMinLabel = ui.Label(fmt(vis.min), {margin: '4px 8px'});
    derivedMaxLabel = ui.Label(fmt(vis.max), {margin: '4px 8px', textAlign: 'right', stretch: 'horizontal'});
    derivedLegendPanel.add(ui.Panel({
      widgets: [derivedMinLabel, derivedMaxLabel],
      layout: ui.Panel.Layout.flow('horizontal')
    }));
  ...
  function updateUncertaintyLegendLabels() {
    ...
    // Use actual min/max for legend labels (stretch stays at mean ± 2σ)
    var minVal, maxVal;
    var hasActualValues = false;
  ...
  // Actual (true) image min/max — used for legend label display only;
  ...
  function makeDerivedCheckbox(productName, isTerrain) {
    var cb = ui.Checkbox({
      label: productName,
      value: productName === 'NDVI',   // NDVI pre-selected by default
      onChange: function(checked) {
        if (checked) {
          // Block terrain products when no matching-year DEM is available
          if (isTerrain && demAvailableForYear === false) {
            derivedCheckboxes[productName].setValue(false, false);
  ...
  // VI split across two columns (4 + 3) so no single column exceeds 4 checkboxes —
  // avoids the GEE layout-engine clipping bug that appears with 7 items in one panel.
  var viProductNames  = ['NDVI', 'EVI', 'ARVI', 'PRI'];
  var viProductNames2 = ['SAVI', 'LAI', 'fPAR'];
  var topoProductNames = ['Slope', 'Aspect', 'Hillshade'];

  // Column 1: first four vegetation indices
  var derivedViColumn = ui.Panel({
    widgets: [
      ui.Label({value: 'Veg. Indices',
                style: {fontSize: '10px', fontStyle: 'italic', color: '#666666', margin: '0 0 2px 0'}})
    ].concat(viProductNames.map(function(n) { return makeDerivedCheckbox(n, false); }))
    .concat([ui.Label({value: '', style: {margin: '0', height: '13px'}})]),
    layout: ui.Panel.Layout.flow('vertical'),
    style: {width: '33%'}
  });

  // Column 2: remaining vegetation indices
  var derivedViColumn2 = ui.Panel({
    widgets: [
      ui.Label({value: '\u00a0',   // non-breaking space keeps header row heights aligned
                style: {fontSize: '10px', margin: '0 0 2px 0'}})
    ].concat(viProductNames2.map(function(n) { return makeDerivedCheckbox(n, false); }))
    .concat([ui.Label({value: '', style: {margin: '0', height: '13px'}})]),
    layout: ui.Panel.Layout.flow('vertical'),
    style: {width: '33%'}
  });

  // Column 3: Terrain Products
  var derivedTopoColumn = ui.Panel({
    widgets: [
      ui.Label({value: 'Terrain',
                style: {fontSize: '10px', fontStyle: 'italic', color: '#666666', margin: '0 0 2px 0'}})
    ].concat(topoProductNames.map(function(n) { return makeDerivedCheckbox(n, true); })),
    layout: ui.Panel.Layout.flow('vertical'),
    style: {width: '33%'}
  });
  ...
  var derivedOptionsPanel = ui.Panel({
    widgets: [
          ...
          ui.Panel([
            ...
            ui.Panel([
              ui.Label({
                ...
                targetUrl: 'https://data.neonscience.org/data-products/DP3.30026.001'
              })
            ]),
            ui.Label({value: ' ', style: {stretch: 'horizontal'}})
          ], ui.Panel.Layout.flow('horizontal'), {stretch: 'horizontal'})
      ...
      ui.Panel({
        widgets: [derivedViColumn, derivedViColumn2, derivedTopoColumn],
        layout: ui.Panel.Layout.flow('horizontal'),
        style: {width: '100%'}
      })
    ],
  ...
  // Warning label shown inside derivedOptionsPanel when no same-year DEM is found
  ...
  derivedOptionsPanel.add(terrainWarningLabel);
  ...
  // Pixel value info panel — non-reflectance, non-RGB images; shown on map click
  ...
  // LAI — NEON ATBD Eq. 3 (DOC.002385): LAI = -(1/a2) * ln((a0 - SAVI) / a1)
  // Validity mask: ln argument must be > 0 (SAVI < LAI_A0); plausibility bounds 0 ≤ LAI ≤ 15.
  function vi_lai(b) {
  ...
  // fPAR — NEON ATBD Eq. 3 (DOC.003840): fPAR = C * [1 - A * exp(-B * LAI)]; clamped to [0, 1]
  function vi_fpar(b) {
    var lai = vi_lai(b);  // inherits water + validity mask from LAI→SAVI
  ...
  function addDerivedProductLayer(image, productName, layerLabel) {
    ...
    // Filter by FLIGHT_YEAR so only same-year DEM tiles are used (Option C).
    var terrainBandMap = {Slope: 'slope', Aspect: 'aspect', Hillshade: 'hillshade'};
    if (terrainBandMap.hasOwnProperty(productName)) {
      ...
      var terrainResult = terrainMosaic.select(terrainBand);
      Map.addLayer(terrainResult, vis, label);
      createDerivedLegend(productName);
      // Asynchronously update legend labels with actual image min/max
      var meta = DERIVED_LEGEND_META[productName];
      terrainResult.reduceRegion({
        reducer: ee.Reducer.minMax(),
        scale: 10,
        maxPixels: 1e9,
        bestEffort: true
        ...
        if (mn !== null && mn !== undefined) derivedMinLabel.setValue(mn.toFixed(meta.decimals) + meta.unit);
        if (mx !== null && mx !== undefined) derivedMaxLabel.setValue(mx.toFixed(meta.decimals) + meta.unit);
      });
      return;
    }
  ...
  function updateMap(selectedImageName1, selectedImageName2, visParams) {
    ...
    // Layer order (bottom → top): MODIS EVI → NLCD → AOP layers
  ...
  function isPixelInfoEligible(collectionName) {
    ...
    if (collectionName === 'Spectrometer Directional Reflectance (HSI_REFL/001)' ||
        collectionName === 'Spectrometer Bidirectional Reflectance (HSI_REFL/002)' ||
        collectionName === 'RGB Camera Photography (RGB/001)') return false;
    if (collectionName === 'Derived Indices (HSI_REFL/002) / Terrain Products (DEM/001)' &&
  ...
  function samplePixelInfo(collectionName, rawImage, imageSlotNum, point, targetPanel) {
    var isDerived = collectionName === 'Derived Indices (HSI_REFL/002) / Terrain Products (DEM/001)';
    var isTerrain = isDerived && (currentDerivedProduct === 'Slope' ||
                                  currentDerivedProduct === 'Aspect' ||
                                  currentDerivedProduct === 'Hillshade');
    ...
    if (collectionName === 'Canopy Height Model (CHM/001)') {
    ...
    } else if (collectionName === 'Digital Surface Model (DEM/001)') {
    ...
    } else if (collectionName === 'Digital Terrain Model (DEM/001)') {
    ...
    } else if (collectionName === 'Canopy Nitrogen Concentration (CNC/002)') {
      var nitroBandDisplay = (imageSlotNum === 1) ? currentNitrogenBand1 : currentNitrogenBand2;
      var cncBandLookup = {
      ...
      normalizeCNCBandNames(rawImage).select(cncMeta.band).reduceRegion({
        ...
        var v = (r && r[cncMeta.band] !== null && r[cncMeta.band] !== undefined)
          ? r[cncMeta.band].toFixed(cncMeta.dec) + cncMeta.unit : 'masked';
        resultLabel.setValue(nitroBandDisplay + ': ' + v);
      });

    } else if (isDerived && isTerrain) {
      var terrainBandKey = {Slope: 'slope', Aspect: 'aspect', Hillshade: 'hillshade'}[currentDerivedProduct];
      var terrainUnit = (currentDerivedProduct === 'Hillshade') ? '' : '\u00b0';
      rawImage.get('FLIGHT_YEAR').evaluate(function(year) {
        var terrainMosaic = ee.ImageCollection('projects/neon-prod-earthengine/assets/DEM/001')
          .filterBounds(rawImage.geometry())
          .filter(ee.Filter.eq('FLIGHT_YEAR', year))
          .select('DTM')
          .map(function(tile) { return ee.Terrain.products(tile); })
          .mosaic();
        terrainMosaic.select(terrainBandKey).reduceRegion({
          reducer: ee.Reducer.first(), geometry: point, scale: 1
        }).evaluate(function(r) {
          var v = (r && r[terrainBandKey] !== null && r[terrainBandKey] !== undefined)
            ? r[terrainBandKey].toFixed(1) + terrainUnit : 'masked';
          resultLabel.setValue(currentDerivedProduct + ': ' + v);
        });
      });

    } else if (isDerived) {
      // Vegetation index: recompute from raw HSI image on the fly
      var viDispatch = {
        NDVI: vi_ndvi, EVI: vi_evi, ARVI: vi_arvi, PRI: vi_pri,
        SAVI: vi_savi, LAI: vi_lai, fPAR: vi_fpar
      };
      var viImage = viDispatch[currentDerivedProduct](vi_precompute(rawImage));
