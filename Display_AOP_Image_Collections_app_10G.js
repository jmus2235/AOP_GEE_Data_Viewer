  ...
  function generateSampleScript() {
    ...
    // — no scriptTemplates entry needed; generate the compute pipeline inline.
    if (selectedCollection === 'Derived Indices (from DP3.30006.001 - Dir. Refl.)' ||
      ...
      hsiImage.toDictionary(['FLIGHT_YEAR', 'CITATION']).evaluate(function(props) {
        ...
        // Veg / water index — include the full compute pipeline
        lines = lines.concat([
    ...
    // Handle Derived Terrain Products — script uses DEM tile directly (no HSI reference needed)
  ...
  function exportCroppedImage() {
    ...
    if (isDerivedExport) {
      ...
      if (terrainBandMapExport.hasOwnProperty(activeProduct1)) {
        // For Derived Terrain Products, image IS the DEM tile — compute terrain directly
  ...
  // Factory: index checkboxes — mutual exclusion within derivedCheckboxes only.
  ...
  // Factory: terrain checkboxes — mutual exclusion within terrainCheckboxes only.
  ...
  // Column 2: biophysical indices (SAVI, LAI, fPAR)
  ...
  // Column 3: water indices (5 items; no terrain here)
  ...
  // Terrain columns — one per product, each 33% wide, in the separate derivedTerrainPanel
  ...
  var derivedOptionsPanel = ui.Panel({
    widgets: [
      ...
      ui.Panel({
        widgets: [derivedViColumn, derivedViColumn2, derivedWiColumn],
        layout: ui.Panel.Layout.flow('horizontal'),
        style: {width: '100%'}
      })
    ],
  ...
  // Separate panel for Derived Terrain Products — shown only when that collection is selected
  ...
  // Warning label — retained for compatibility; not triggered in current design
  ...
  var VI_BAND_RANGES = {
    ...
    wbi1:  [892.5,  907.5],   // ρ900  (WBI denominator)
    wbi2:  [962.5,  977.5],   // ρ970  (WBI numerator)
    nmdi1: [841.0,  876.0],   // ρ860
    nmdi2: [1628.0, 1652.0],  // ρ1640
    nmdi3: [2105.0, 2155.0],  // ρ2130
    ndwi1: [849.5,  864.5],   // ρ857
    ndwi2: [1232.5, 1247.5],  // ρ1241
    ndii1: [811.5,  826.5],   // ρ819  (shared with MSI)
    ndii2: [1640.5, 1656.5],  // ρ1649
    msi2:  [1591.5, 1606.5]   // ρ1599
  };
  ...
  var DERIVED_VIS = {
    ...
    // Water indices (NEON.DOC.004364) — higher = more moisture stress for WBI/MSI;
  ...
  // ----------------------------
  // Canopy Water Indices — NEON.DOC.004364 (DP3.30019.001), Eqs. 1–5
  ...
  // NMDI = (ρ860 − (ρ1640 − ρ2130)) / (ρ860 + (ρ1640 − ρ2130))  (Eq. 2)
  ...
  // NDWI = (ρ857 − ρ1241) / (ρ857 + ρ1241)  (Eq. 3)
  ...
  // NDII = (ρ819 − ρ1649) / (ρ819 + ρ1649)  (Eq. 4)
  ...
  function samplePixelInfo(collectionName, rawImage, imageSlotNum, point, targetPanel) {
    ...
    if (collectionName === 'Canopy Height Model (DP3.30015.001)') {
      ...
      // rawImage IS the DEM tile — compute terrain directly
